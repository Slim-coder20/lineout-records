...
export default function SocialLinks({
  ...
  return (
    <ul className={`flex items-center gap-2 ${justifyClass} ${className}`}>
      {socialLinks.map(({ href, label }) => (
        <li key={label}>
          <a
            ...
            aria-label={label}
            className="flex size-10 items-center justify-center rounded-full bg-brand-dark/10 text-brand-dark transition-colors hover:bg-brand-accent/30 hover:text-brand-dark/80"
          >
            {icons[label]}
          </a>
        </li>
